...
begin
    ...
    Vpd_m = Vpd_a + (Δ * A - (Δ + γ) * λE) * r_aa / (ρ * cp) # Vapor pressure deficit at source height, (7) of Lh12
    λE_s = (Δ * As + ρ * cp * Vpd_m / r_as) / (Δ + γ * (1 + r_ss / r_as)) # Latent heat flux from the soil
    λE_c = (Δ * Ac + ρ * cp * Vpd_m / r_ac) / (Δ + γ * (1 + r_sc / r_ac)) # Latent heat flux from the canopy
...
begin
    λE_t = (1 - f_wet) * (Δ * Ac + ρ * cp * Vpd_m / r_ac) / (Δ + γ * (1 + r_sc / r_ac))
    λE_i = f_wet * (Δ * Ac + ρ * cp * Vpd_m / r_ac) / (Δ + γ)
...
begin
    ...
    λE_check =
        (X * r_aa / DE) * ((Δ + γ) / γ) * λEp +
        (Δ / γ) *
        ((Rc * Ri * As * r_as + r_ac * Ac * ((1 - f_wet) * Rs * Ri + f_wet * Rc * Rs)) / DE)
end
...
begin
    ...
    λE_check_2 =
        simplify(((Δ + γ) / γ) * (Pc_ + Pi_ + Ps_) * λEp) +
...
begin
    ...
    λE_check_alt =
        simplify(((Δ + γ) / γ) * (Pcstar + Psstar) * λEp) +
...
begin
    λE_sp = (Δ * As + ρ * cp * Vpd_m / r_as) / (Δ + γ)
