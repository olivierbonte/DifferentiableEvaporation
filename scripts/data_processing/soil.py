...
if not os.path.exists(conf_module.soil_pro_dir):
    os.makedirs(conf_module.soil_pro_dir)

# %% Datacube creation
for site in conf_module.sites:
    print(f"site in progress: {site}")
    ds_soilgrids = xr.open_dataset(
        glob.glob(os.path.join(conf_module.soilgrids_dir, site, "*.nc"))[0],
        decode_coords="all",
    )
    ds_hihydrosoil = xr.open_dataset(
        os.path.join(conf_module.hihydrosoil_dir, site + ".nc"), decode_coords="all"
    )
    ...
    ds_soil.to_netcdf(os.path.join(conf_module.soil_pro_dir, site + "_soil_cube.nc"))

    # Spatial data aggregation
    ds_soil_agg = ds_soil.mean(dim=["x", "y"], keep_attrs=True)
    ds_soil_agg.to_netcdf(
        os.path.join(conf_module.soil_pro_dir, site + "_soil_horizontal_agg.nc")
    )

    # Add vertical aggregation
